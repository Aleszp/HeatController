...
int main(void)
{
	...
	while(1)
	{
		...
		if(flags0&(1<<PRINT_K))				//jeśli ustawiono flagę wyświetlenia współczynników PID (ustawiana w przerwaniu na żądanie użytkownika)
		{
			...
			USART_Append_To_Buffer_int8(Kp,1,1);
			USART_Append_To_Buffer(("Ki="),3,1);
			USART_Append_To_Buffer_int8(Ki,1,1);
			USART_Append_To_Buffer(("Kd="),3,1);
			USART_Append_To_Buffer_int8(Kd,1,1);
			USART_Append_To_Buffer(("M="),2,1);
			USART_Append_To_Buffer_int8(denominator,0,0);
		...
		if(flags1&(1<<HELP))				//jeśli ustawiono flagę wyświetlenia pomocy
		{
			...
			USART_Append_To_Buffer(("Pxxx - ustaw Kp = xxx\n"),22,1);
			USART_Append_To_Buffer(("Cxxx - ustaw Ki = xxx\n"),22,1);
			USART_Append_To_Buffer(("Qxxx - ustaw Kd = xxx\n"),22,1);
			USART_Append_To_Buffer(("Mxxx - ustaw mianownik = xxx\n"),29,1);
